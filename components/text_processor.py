...
def render_text_processor() -> None:
    ...
    input_text = st.text_area("Input Text", placeholder=f"Paste your text with marked terms...\n\nExample: {syntax_info.get('example', '')}", height=200)
    col1, col2 = st.columns([1, 3])
    with col1:
        output_format = st.selectbox("Output Format", options=["telegram", "markdown", "html"], format_func=lambda x: {"telegram": "Telegram", "markdown": "Markdown", "html": "HTML"}.get(x, x))
    with col2:
        st.markdown("")
    ...
    if process_button and input_text:
        ...
        _display_result(st.session_state.processed_result, st.session_state.found_terms, st.session_state.missing_terms, output_format)


def _process_and_display(text: str, syntax: str, output_format: str, custom_prefix: str = "", custom_suffix: str = "") -> None:
...
def _display_result(processed: str, found_terms: list, missing_terms: list, output_format: str) -> None:
    st.divider()
